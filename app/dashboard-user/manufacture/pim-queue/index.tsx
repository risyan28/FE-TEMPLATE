...
const LINE_MAP: Record<string, string> = {
  'LINE-A': 'Cylinder Block Line',
  'LINE-B': 'Sub Assy Line',
  'LINE-C': 'Engine Line',
  'LINE-D': 'Cylinder Head Line',
  'LINE-E': 'Crankshaft Line',
  'LINE-F': 'Camshaft Line',
}
...
function generateQueueData(): PimSequence[] {
  const data: PimSequence[] = []
  const shiftDate = '2025-12-26'

  const COMPLETED_UNTIL = 6
  const PROCESSING_UNTIL = 7
  ...
  for (let i = 1; i <= 20; i++) {
    ...
    const seqOri = String(i + 909).padStart(3, '0')
    const seqAdj = String(i + 889).padStart(3, '0')
    const isManual = i >= 19
    const seqMan = isManual ? `RW00${i - 18}` : null
...
const QUEUE_DATA = generateQueueData()
...
function StatCard({
  ...
  return (
    <motion.div
      ...
      className={`flex-shrink-0 min-w-[120px] sm:min-w-[180px] flex-1 rounded-lg border ${borderColors[accent ?? 'default']} bg-gradient-to-br ${bgGradients[accent ?? 'default']} p-2 sm:p-2.5 shadow-sm hover:shadow-md transition-all duration-200`}
    >
...
export function PimQueuePage() {
  const [searchParams] = useSearchParams()
  const lineCode = searchParams.get('line') ?? 'LINE-A'
  const lineName = LINE_MAP[lineCode] ?? lineCode
  ...
  const TARGET_HV = 10
  const TARGET_CONV = 10

  const total = QUEUE_DATA.length
  const hv = QUEUE_DATA.filter((s) => s.egPower === 'HV').length
  const conv = QUEUE_DATA.filter((s) => s.egPower === 'CONV').length

  const hvCompleted = QUEUE_DATA.filter(
    (s) => s.egPower === 'HV' && s.printFlag === 2,
  ).length
  const convCompleted = QUEUE_DATA.filter(
    (s) => s.egPower === 'CONV' && s.printFlag === 2,
  ).length
  const totalCompleted = hvCompleted + convCompleted

  const filteredData = useMemo(() => {
    return QUEUE_DATA.filter((row) => {
  ...
  return (
    <div className='fixed inset-0 flex flex-col bg-background'>
      <AppHeader title={`${lineName} - ${lineCode}`} className='h-14 md:h-14' />

      {/* Summary */}
      <div className='flex-shrink-0 px-2 md:px-6 pb-2 pt-2 md:pt-2'>
        <motion.div
        ...
        >
          <div className='flex gap-2 sm:gap-3 flex-1 min-w-0 overflow-x-auto sm:overflow-visible scrollbar-hide'>
            <StatCard
              label='Total Target'
              value={TARGET_TOTAL}
...
{/*           <div className='flex flex-col items-end justify-end gap-1 shrink-0'>
      ...
      <div className='flex-1 min-h-0 px-2 md:px-6 pb-4 flex flex-col overflow-hidden'>
        <div className='flex-shrink-0 flex items-center gap-2 mb-2 text-xs text-muted-foreground/70 font-medium'>
          ...
          <span>Sequence PI Machining</span>
        ...
        {/* Filters bar */}
        <div className='flex-shrink-0 flex flex-col md:flex-row md:flex-wrap md:items-center gap-2 mb-2 pb-2 border-b border-border/40'>
          {/* Row 1 (mobile): status tabs + power select */}
          ...
          {/* Row 2 (mobile): search + show + pagination */}
          <div className='flex items-center gap-2 md:hidden'>
          ...
          {/* Row 3 (mobile): inject button full width */}
          <div className='flex items-center md:hidden'>
            <button
              onClick={() => setInjectOpen(true)}
              className='w-full flex items-center justify-center gap-1.5 rounded-lg bg-blue-600 px-3 py-2 text-[0.65rem] font-semibold text-white shadow-sm hover:bg-blue-700 transition-colors'
            >
              <Plus className='h-3 w-3' />
              Inject Manual Sequence
            </button>
          </div>

          {/* Desktop: all in 1 row */}
          ...
          <div className='hidden md:flex items-center gap-2 shrink-0'>
            <button
              onClick={() => setInjectOpen(true)}
              className='flex items-center gap-1.5 rounded-lg bg-blue-600 px-3 py-1.5 text-[0.65rem] font-semibold text-white shadow-sm hover:bg-blue-700 transition-colors'
            >
              <Plus className='h-3 w-3' />
              Inject Sequence Manual
            </button>
